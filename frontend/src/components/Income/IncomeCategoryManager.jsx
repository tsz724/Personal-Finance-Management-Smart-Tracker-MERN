...
const IncomeCategoryManager = ({ categories, onUpdated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await axiosInstance.put(API_PATHS.MODULES.FINANCE.INCOME_CATEGORIES.UPDATE(editingId), {
          name: trimmed,
          icon: icon || undefined,
        });
        toast.success("Category updated");
      } else {
        await axiosInstance.post(API_PATHS.MODULES.FINANCE.INCOME_CATEGORIES.CREATE, {
          name: trimmed,
          icon: icon || undefined,
        });
  ...
  const handleDelete = async (id) => {
    try {
      await axiosInstance.delete(API_PATHS.MODULES.FINANCE.INCOME_CATEGORIES.DELETE(id));
  ...
  return (
    <Box sx={{ px: 1, py: 1, pb: 3 }}>
      <Paper variant="outlined" sx={{ p: 2, mb: 3, borderRadius: 2 }}>
        ...
        <TextField label="Name" placeholder="Salary, freelance…" value={name} onChange={(e) => setName(e.target.value)} margin="normal" fullWidth />
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete category">
        <Deletealert
          content="Delete this category? You can only delete categories that are not used by any income entry."
